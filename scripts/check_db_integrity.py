...
# Define target tables and JSON file mapping
TABLE_JSON_MAPPING = {
...
def get_json_object_count(filepath: str) -> int:
    """Parses JSON and counts the number of items, accounting for structure."""
    with open(filepath, "r") as f:
        data = json.load(f)
        if isinstance(data, list):         # e.g. [ {...}, {...} ]
            return len(data)
        elif isinstance(data, dict):
            if "data" in data and isinstance(data["data"], list):  # e.g. { "data": [ {...} ] }
...
def main():
    print("\n🔍 Comparing JSON files vs DB table counts...\n")
